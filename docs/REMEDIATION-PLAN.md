...
- [ ] **Ground-truth evaluation harness**: small labeled sets per capability (bot/no-bot, propaganda/normal, fact-checked claims) + a script reporting precision/recall so quality claims are measurable. Validate bot detection against a public bot dataset. *(Also the validation vehicle for the causal-inference layer below — see that doc's §7.)*
- [ ] **Analysis quality across interpretive layers** — every analysis service ranks on hand-tuned, unvalidated constants (bot detection `0.3/0.3/0.4`, clustering threshold `0.75`, facet multipliers `0.72/1.08/0.68`, credibility `0.7/0.3`). Bot detection asks an unanswerable question (automation) instead of a measurable one (coordination); narrative clustering has no stance axis, so "ban X" and "banning X is tyranny" merge. Full audit, research grounding, phased plan and the interactive parameter-control design: **[`development/analysis-quality-plan.md`](./development/analysis-quality-plan.md)**. Phase A (stop over-claiming) is small and independently shippable.
- [ ] **Causal inference rigor** — the downstream-effects layer asserts `caused`/`caused_by` from temporal ordering alone (`downstream-effects.service.ts:617-623`), ranks by a score in which signal magnitude is 30% of "correlation strength" (`:609`), and filters at `> 0.1` (`:641`) which magnitude alone clears. No base rate, no lag constraint, no multiple-comparison control; the agentic path's `overall_confidence` is LLM-self-reported. Full design, phased plan, and validation strategy: **[`development/causal-inference-layer.md`](./development/causal-inference-layer.md)**. Phase 0 (stop asserting causation from ordering) is small and independently shippable.
